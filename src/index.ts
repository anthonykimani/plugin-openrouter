...
export const anthropicPlugin: Plugin = {
  name: 'openrouter',
  description: 'OpenRouter plugin (supports text generation only)',
  config: {
  ...
  async init(config: Record<string, string>) {
    try {
      ...
      logger.warn(
        `OpenRouter plugin configuration issue: ${error.errors
          .map((e) => e.message)
          .join(', ')} - You need to configure the OPENROUTER_API_KEY in your environment variables`
      );
  ...
  models: {
    [ModelType.TEXT_SMALL]: async (runtime, { prompt, stopSequences = [] }: GenerateTextParams) => {
      ...
      const smallModel = runtime.getSetting('OPENROUTER_SMALL_MODEL') ?? 'anthropic/claude-3-haiku';
      const maxTokens = smallModel.includes('-3-') ? 4096 : 8192;

      const { text } = await generateText({
        model: openrouter.chat(smallModel),
        prompt,
    ...
    [ModelType.TEXT_LARGE]: async (
      ...
      const largeModel = runtime.getSetting('OPENROUTER_LARGE_MODEL') ?? 'anthropic/claude-3.5-sonnet';

      const { text } = await generateText({
        model: openrouter.chat(largeModel),
        prompt,
        system: runtime.character.system ?? undefined,
        temperature,
        maxTokens,
    ...
    [ModelType.OBJECT_LARGE]: async (runtime, params: ObjectGenerationParams) => {
      const largeModel = runtime.getSetting('ANTHROPIC_LARGE_MODEL') ?? 'anthropic/claude-3.5-sonnet';
      try {
        // Check if this is a reflection schema request (has specific format)
        const isReflection = params.schema?.facts && params.schema.relationships;
